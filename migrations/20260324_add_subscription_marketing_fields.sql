-- Migration: Add Subscription Marketing Fields
...
ON pro_subscriptions(reminder_sent_at, status, created_at)
WHERE status = 'payment_pending';

CREATE INDEX IF NOT EXISTS idx_influencers_last_nudge_sent
ON influencers(last_nudge_sent_at, is_pro, is_verified)
WHERE is_pro = false AND is_verified = true;
