...
export class BrandController {
  ...
  @Get('wallet')
  @ApiOperation({
    summary: 'Get brand wallet balance',
    description: 'Get brand wallet balance including available and locked amounts',
  })
  @ApiResponse({
    status: 200,
    ...
    schema: {
      ...
      properties: {
        ...
        data: {
          ...
          properties: {
            id: { type: 'number', example: 1 },
            availableBalance: { type: 'number', example: 5000.00, description: 'Balance available for use' },
            lockedAmount: { type: 'number', example: 500.00, description: 'Amount locked during return windows' },
            totalAmount: { type: 'number', example: 5500.00, description: 'Total of available + locked' },
            totalCredited: { type: 'number', example: 10000.00, description: 'Total amount ever credited (recharges)' },
            totalDebited: { type: 'number', example: 5000.00, description: 'Total amount debited (cashback payouts)' },
            isActive: { type: 'boolean', example: true },
            createdAt: { type: 'string', example: '2026-03-01T10:00:00.000Z' },
            updatedAt: { type: 'string', example: '2026-04-23T10:00:00.000Z' },
          },
        },
  ...
  @ApiResponse({ status: 400, description: 'Only brands can view their wallet' })
  @ApiResponse({ status: 401, description: 'Unauthorized' })
  async getBrandWallet(@Req() req: RequestWithUser) {
    if (req.user.userType !== 'brand') {
      throw new BadRequestException('Only brands can view their wallet');
    }
    return this.brandService.getBrandWallet(req.user.id);
  }

  @Get('wallet/transactions')
  @ApiOperation({
    summary: 'Get brand wallet transaction history',
    description: 'Get paginated wallet transaction history for the brand. Includes recharges, cashback payouts to influencers, refunds, and admin adjustments.',
  })
  ...
  @ApiQuery({
    name: 'type',
    required: false,
    enum: ['recharge', 'debit', 'refund', 'adjustment'],
            ...
            transactions: {
              ...
              items: {
                ...
                properties: {
                  ...
                  balanceBefore: { type: 'number', example: 5250.00 },
                  balanceAfter: { type: 'number', example: 5000.00 },
                  status: {
                    type: 'string',
                    example: 'completed',
                    enum: ['pending', 'processing', 'completed', 'failed', 'cancelled'],
                  },
                  description: { type: 'string', example: 'Cashback payout for order ORD123456' },
                  hypeStoreId: { type: 'number', nullable: true, example: 3 },
                  hypeStoreOrderId: { type: 'number', nullable: true, example: 388 },
                  relatedUserId: { type: 'number', nullable: true, example: 42, description: 'Influencer ID who received cashback' },
                  createdAt: { type: 'string', example: '2026-04-23T10:00:00.000Z' },
                },
              },
            },
            pagination: {
  ...
  async getBrandWalletTransactions(
    ...
    return this.brandService.getBrandWalletTransactions(
      req.user.id,
      page ? Number(page) : 1,
