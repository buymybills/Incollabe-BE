...
@Controller('api/influencer/hype-store')
@UseGuards(AuthGuard)
@ApiBearerAuth()
export class InfluencerHypeStoreController {
  ...
  @Get('my-coupons')
  @ApiOperation({
    summary: 'Get all my coupon codes',
    description: 'Get all coupon codes generated by this influencer across all Hype Stores'
  })
  @ApiResponse({
    status: 200,
        ...
        data: {
          ...
          properties: {
            universalCoupon: {
              type: 'object',
              nullable: true,
              description: 'Your universal coupon that works for ALL stores',
              properties: {
                id: { type: 'number', example: 1 },
                couponCode: { type: 'string', example: 'INFL123' },
                isUniversal: { type: 'boolean', example: true },
                hypeStoreId: { type: 'number', nullable: true, example: null },
                isActive: { type: 'boolean', example: true },
                totalUses: { type: 'number', example: 25 },
                createdAt: { type: 'string', example: '2026-03-07T10:00:00.000Z' },
              },
            },
            storeCoupons: {
              type: 'array',
              description: 'Legacy store-specific coupons (for backward compatibility)',
              items: {
                type: 'object',
                properties: {
                  ...
                  isUniversal: { type: 'boolean', example: false },
                  isActive: { type: 'boolean', example: true },
            ...
            allCoupons: {
              type: 'array',
              description: 'All coupons (universal + store-specific)',
              items: { type: 'object' },
            },
          },
        },
        message: { type: 'string', example: 'Coupon codes retrieved successfully' },
      },
    },
  })
  @ApiResponse({ status: 401, description: 'Unauthorized' })
  async getMyCoupons(@Request() req: any) {
    const influencerId = req.user.id;
    return this.hypeStoreService.getMyCoupons(influencerId);
  }
  ...
  @Post(':storeId/generate-coupon')
  @HttpCode(HttpStatus.OK)
  @ApiOperation({
    summary: '[DEPRECATED] Generate store coupon - Use GET /my-coupon instead',
    description:
      '⚠️ DEPRECATED: This endpoint is deprecated. ' +
      'Use GET /api/influencer/hype-store/my-coupon instead. ' +
      'We now use ONE universal coupon per influencer that works for ALL stores.',
    deprecated: true,
  })
  @ApiParam({ name: 'storeId', type: Number, description: 'Store ID (ignored - returns universal coupon)' })
  @ApiResponse({
    status: 200,
    description: 'Returns universal coupon (not store-specific)',
    schema: {
      type: 'object',
      properties: {
        success: { type: 'boolean', example: true },
        ...
        message: { type: 'string', example: 'Coupon generated successfully' }
      }
    }
  })
  @ApiResponse({ status: 404, description: 'Store not found' })
  ...
  async generateCoupon(
    ...
    const influencerId = req.user.id;
    return this.hypeStoreService.generateCouponCode(influencerId, parseInt(storeId));
  }
